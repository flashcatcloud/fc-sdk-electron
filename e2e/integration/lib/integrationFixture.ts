...
export async function launchApp(
  ...
  if (mode === 'packaged') {
    return electron.launch({
      ...
      args: userDataArgs,
      env: { ...process.env, FC_SDK_CONFIG: JSON.stringify(config) } as Record<string, string>,
    });
  }
...
function buildSdkConfig(intake: Intake): InitConfiguration {
  return {
    // `site` omitted on purpose — it is optional and resolves to DEFAULT_SITE; `proxy` decides
    // where uploads actually go. Same choice as the core e2e fixture.
